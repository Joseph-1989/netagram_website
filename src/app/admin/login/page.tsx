...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await graphqlRequest<{ adminLogin: { token: string } }>(
        LOGIN_MUTATION,
        {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg border border-gray-100">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900 bg-gradient-to-r from-teal-500 to-teal-400 bg-clip-text text-transparent">
            Admin Login
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            Sign in to manage Update Information
          </p>
        </div>
